...
public final class IndexDitaProcessor {
  ...
  public List<IndexEntry> processIndexDitaNode(final Node node, final String parentValue) {
        ...
        case Node.ELEMENT_NODE:
          if (TOPIC_INDEXTERM.matches(child)) {
            ...
            final String currentRefId =
                currentTextValue.isEmpty() ? "" : (currentTextValue + VALUE_SEPARATOR);
    ...
    if (textValue.contains(SORT_START)
        && textValue.contains(SORT_END)
        && sortString.length() == 0) {
      if (textValue.indexOf(SORT_START) < textValue.indexOf(SORT_END)) {
        sortString =
            textValue.substring(textValue.indexOf(SORT_START) + 1, textValue.indexOf(SORT_END));
    ...
    if (!childEntrys.isEmpty() && !seeEntry.isEmpty()) {
      for (final IndexEntry e : seeEntry) {
        logger.warn(
            MessageUtils.getMessage("DOTA067W", e.getFormattedString(), textValue).toString());
      }
      seeEntry.clear();
    }
    if (!childEntrys.isEmpty() && !seeAlsoEntry.isEmpty()) {
      for (final IndexEntry e : seeAlsoEntry) {
        logger.warn(
            MessageUtils.getMessage("DOTA068W", e.getFormattedString(), textValue).toString());
      }
      seeAlsoEntry.clear();
    }

    final IndexEntry result =
        new IndexEntryImpl(
            textValue, sortString.isEmpty() ? null : sortString, textValue, contents);
